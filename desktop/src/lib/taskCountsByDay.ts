...
export function clampWindowToDay(start: Date, end: Date, day: Date): { start: Date; end: Date } | null {
  ...
  return { start: start < dayStart ? dayStart : start, end: end > dayEnd ? dayEnd : end };
}

export function countTasksByDay(tasks: CountableTask[], start: Date, end: Date): Record<string, DayDensity> {
  ...
  for (const task of tasks) {
    ...
    for (const field of ["startDate", "deadline"] as const) {
      ...
      const date = new Date(task[field] as string);
