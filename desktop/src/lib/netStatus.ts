 ...
 *
 *   1. The browser online state, pushed in by `useConnectivity` from `navigator.onLine`.
 *   2. The outcome of real requests, pushed in by the API client.
 *
 * Both are needed. `navigator.onLine` tells us the machine has a network
 * interface, but it cannot tell us the backend is unreachable — a captive portal, a dead backend,
...
export interface NetStatus {
  /** Browser online state. `null` until `useConnectivity` reports in. */
...
/** Called by `useConnectivity` with the `navigator.onLine` state. */
export function reportRadioState(online: boolean) {
  if (state.radioOnline === online) return;
  // Regaining the network makes our last request-derived verdict stale: we have
