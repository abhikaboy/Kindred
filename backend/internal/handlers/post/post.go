...
func (h *Handler) AddReactionHuma(ctx context.Context, input *AddReactionInput) (*AddReactionOutput, error) {
	user_id, err := auth.RequireAuth(ctx)
	if err != nil {
	...
	if err := h.service.AddReaction(reaction); err != nil {
		return nil, huma.Error500InternalServerError("Failed to add reaction", err)
	}
