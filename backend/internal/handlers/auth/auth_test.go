...
func TestAuthorizationHeaderValidation(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			shouldFail:  false,
			description: "Should pass with valid Bearer token format",
		},
		{
			name:        "Empty authorization header",
			authHeader:  "",
			shouldFail:  true,
			description: "Should fail with empty authorization header",
		},
		{
			name:        "Missing Bearer prefix",
			authHeader:  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9",
			shouldFail:  true,
			description: "Should fail without Bearer prefix",
		},
		{
			name:        "Wrong token type",
			authHeader:  "Basic eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9",
			shouldFail:  true,
			description: "Should fail with Basic instead of Bearer",
		},
		{
			name:        "Only Bearer without token",
			authHeader:  "Bearer",
			shouldFail:  true,
			description: "Should fail with Bearer but no token",
		},
		{
			name:        "Bearer with empty token",
			authHeader:  "Bearer ",
			shouldFail:  true,
			description: "Should fail with Bearer and empty token",
		},
		{
		...
		},
		{
			...
			shouldFail:  true,
			description: "Should fail with extra parts after token",
		},
		{
			name:        "Case sensitive Bearer",
			authHeader:  "bearer token",
			shouldFail:  true,
			description: "Should fail with lowercase bearer",
		},
		{
			name:        "Mixed case Bearer",
			authHeader:  "BeArEr token",
			shouldFail:  true,
			description: "Should fail with mixed case Bearer",
		},
	}

	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			// Simulate the header parsing logic from LogoutHuma
			if len(tc.authHeader) == 0 {
				if tc.shouldFail {
					assert.Empty(t, tc.authHeader, tc.description)
				} else {
					assert.NotEmpty(t, tc.authHeader, tc.description)
				}
				return
			}
			...
			if tc.shouldFail {
				...
				if len(split) != 2 {
					assert.NotEqual(t, 2, len(split), tc.description)
				} else if split[0] != "Bearer" {
					assert.NotEqual(t, "Bearer", split[0], tc.description)
				} else if len(split[1]) == 0 {
					assert.Empty(t, split[1], tc.description)
				}
			} else {
				// Check success conditions
				assert.Equal(t, 2, len(split), tc.description)
				assert.Equal(t, "Bearer", split[0], tc.description)
				assert.NotEmpty(t, split[1], tc.description)
			}
		})
	}
}
