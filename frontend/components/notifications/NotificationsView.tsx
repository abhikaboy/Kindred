...
type NotificationsViewProps = {
    /** Whether this view is the currently-visible pager page. Drives mark-as-read. */
    isActive: boolean;
    /** Invoked by the header back-arrow (pager: go to feed page; route: dismiss to feed). */
    onBack: () => void;
};

const NotificationsView = ({ isActive, onBack }: NotificationsViewProps) => {
    ...
    // Mark all notifications as read when this page becomes the active pager page.
    // Runs at most once per activation, only if there's something unread, and never
    // re-fires on render (markAllAsRead is intentionally not in the deps — its identity
    // changed every render in an earlier revision and triggered a request-spam loop).
    // The guard resets when the page deactivates, so the next activation re-marks.
    ...
    useEffect(() => {
        if (isActive && !loading && hasUnread && !hasMarkedAsRead.current) {
